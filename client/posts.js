...
export async function getPostBySlug(slug) {
  ...
  const apiHost = new URL('https://stoppopulationdecline.org/graphql').host;
...
export async function getPaginatedPosts({ currentPage = 1, queryIncludes = 'index', ...options } = {}) {
  const { posts } = await getAllPosts({queryIncludes, ...options});
